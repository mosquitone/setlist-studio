...
export class EmailRateLimit {
  ...
  async checkEmailSendLimit(
    ...
    if (!result.success) {
      ...
      return {
        ...
        message: options.message || 'メール送信回数が上限に達しました。しばらく時間をおいてから再試行してください。',
      };
    }
  ...
  async checkIPEmailLimit(
    ...
    if (!result.success) {
      ...
      return {
        ...
        remaining: result.remaining,
        resetTime: result.resetTime,
        message: 'このIPアドレスからのメール送信回数が上限に達しました。しばらく時間をおいてから再試行してください。',
      };
    }
  ...
  async checkPasswordResetLimit(
    ...
    // メールアドレスベースの制限
    const emailResult = await this.checkEmailSendLimit(email, 'password_reset', ipAddress, userAgent);
   ...
   * メール種別ごとのレート制限オプション
   */
  private getEmailRateLimitOptions(type: 'verification' | 'password_reset' | 'email_change'): EmailRateLimitOptions {
    ...
    switch (type) {
      case 'password_reset':
        return {
          ...
          message: 'パスワードリセットメールの送信回数が上限に達しました。1時間後に再試行してください。',
        };
      case 'verification':
        return {
      ...
      case 'email_change':
        return {
          ...
          message: 'メールアドレス変更メールの送信回数が上限に達しました。1時間後に再試行してください。',
        };
      default:
        return {
   ...
   */
  private getIPEmailRateLimitOptions(type: 'verification' | 'password_reset' | 'email_change'): EmailRateLimitOptions {
    const isDevelopment = process.env.NODE_ENV === 'development';
